...
namespace AppAfpaBrive.Web.Helpers.CalendarTagHelper
{
    ...
    public class CalendarTagHelper : TagHelper
    {
        ...
        public ICollection<Evenement> Events { get; set; }
        ...
        private string GetHtml()
        {
            ...
            var events = Events?.GroupBy(e => e.DateEvent);
            ...
            var html = new XDocument(
                new XElement("div",
                    ...
                    new XElement("header",
                    new XElement("button",
                                new XAttribute("type", "button"), new XAttribute("class", "btn btn-primary"), new XAttribute("id", "btnP"), "<"
                        ),
                       new XElement("button",
                                new XAttribute("type", "button"), new XAttribute("class", "btn btn-primary"), new XAttribute("id", "btnS"), ">"
                        ),
                        new XElement("h4",
                            new XAttribute("class", "display-4 mb-2 text-center"),
                            new XAttribute("id", "titre"),
            ...
            IEnumerable<XElement> GetEventHtml(DateTime d)
            {
                return events?.SingleOrDefault(e => e.Key == d)?.Select(e =>
                    new XElement("a",
                        ...
                        new XAttribute("title", e.DétailsEvent),
                        e.DétailsEvent
                    )
                ) ?? new[] {
                new XElement("p",
